...
function useAudio() {
  ...
  const tone = useCallback((freq, dur, type = "sine", vol = 0.045, slide = 0) => {
    try {
      const c = ensure();
      if (!c) return;
  ...
  return {
    tap: useCallback(() => tone(920, 0.07, "sine", 0.035, -400), [tone]),
    success: useCallback(() => { tone(523, 0.1); setTimeout(() => tone(784, 0.14), 80); }, [tone]),
    deny: useCallback(() => tone(160, 0.22, "triangle", 0.05, -40), [tone]),
    whoosh: useCallback(() => tone(240, 0.18, "sine", 0.02, 600), [tone]),
  };
}
...
/* ─── Hero section separated so useScroll ref is always mounted ─── */
function HeroSection({ onEnroll, onLogin, sfx }) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        style={{ y: heroY, opacity: heroOp, textAlign: "center", maxWidth: 980, padding: "0 48px", position: "relative" }}
      >
        <motion.div
        ...
        >
          ...
          <motion.button
            ...
            style={{
              ...
              color: "#042f2e", border: "none", borderRadius: 999, padding: "16px 32px",
              fontWeight: 700, fontSize: 13, letterSpacing: "0.06em", cursor: "pointer",
              display: "inline-flex", alignItems: "center", gap: 10,
              boxShadow: "0 10px 40px rgba(45,212,191,0.25)", textTransform: "uppercase",
            }}
            onClick={onLogin}
          >
...
export default function App() {
  ...
  const [enrolled, setEnrolled] = useState(() => {
    try { return JSON.parse(localStorage.getItem("ss_pc_enr") || "[]"); } catch { return []; }
  });
  const [audit, setAudit] = useState(() => {
    try { return JSON.parse(localStorage.getItem("ss_pc_aud") || "[]"); } catch { return []; }
  });
  ...
  useEffect(() => {
    try { localStorage.setItem("ss_pc_enr", JSON.stringify(enrolled)); } catch {}
  }, [enrolled]);

  useEffect(() => {
    try { localStorage.setItem("ss_pc_aud", JSON.stringify(audit)); } catch {}
  }, [audit]);
  ...
  const snap = useCallback(() => {
    ...
    if (modelsOk && !faceOn && !camErr) {
      alert("No face detected — centre your face in the guide.");
      return;
    }
    if (camErr) {
    ...
    try { sfx.success(); } catch {}
  }, [sfx, captures.length, faceOn, camErr, modelsOk]);

  const finishEnrol = useCallback(() => {
    try { sfx.tap(); } catch {}
    ...
    stopCam();
    try { sfx.success(); } catch {}
  }, [sfx, captures, form, stopCam]);

  const runScan = useCallback(async () => {
    try { sfx.whoosh(); } catch {}
  ...
  }, [sfx, enrolled, anomalous, fails, startCam, stopCam]);
    ...
    <header style={{
      ...
      >
        ...
        <div>
          ...
          <div style={{ fontSize: 10, color: T.gold, letterSpacing: "0.28em", marginTop: 3 }}>
            HOSPITAL · SECURITY OS
          </div>
        </div>
      </div>
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <span style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: 12, color: T.dim, marginRight: 12 }}>
          {clock}
        </span>
        {right}
      </div>
        ...
        <div style={{ position: "relative", zIndex: 2 }}>
          ...
          >
            <motion.div variants={fadeUp} style={{ textAlign: "center", marginBottom: 64 }}>
              <div style={{ color: T.gold, fontSize: 11, letterSpacing: "0.3em", fontWeight: 700, marginBottom: 16 }}>
                ARCHITECTURE
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
              {[
                {
                  i: Camera, n: "01", t: "Liveness Scan",
                  d: "Real webcam face-presence detection confirms a living subject before scoring begins — not a static photo spoof.",
                },
                {
                  i: Activity, n: "02", t: "Risk Intelligence",
                  d: "Five weighted signals: device, network geofence, time-of-day, failure pressure, and biometric confidence.",
                },
                {
                  i: ShieldCheck, n: "03", t: "Governed Access",
                  d: "Trusted entry, step-up OTP, or hard deny with incident log. Every decision is explainable for audit.",
                },
              ].map((c) => (
                <motion.div
                  key={c.n}
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
                    <div style={{
                      width: 52, height: 52, borderRadius: 16, background: T.goldDim,
          ...
          >
            <div style={{
              maxWidth: 1200, margin: "0 auto", padding: "56px",
              ...
              {[
                ...
                <motion.div key={i} variants={fadeUp} style={{ textAlign: "center" }}>
                  <div style={{
                    fontSize: 48, fontWeight: 600, color: T.gold,
                    fontFamily: "IBM Plex Mono, monospace", letterSpacing: "-0.04em",
                  }}>
                    {x.v}
                  </div>
                  <div style={{
                    fontSize: 12, color: T.dim, letterSpacing: "0.16em",
          ...
          >
            <motion.div variants={fadeUp} style={{ marginBottom: 48 }}>
              ...
              <h2 style={{ fontSize: 40, fontWeight: 500, letterSpacing: "-0.03em", maxWidth: 520 }}>
                Everything a hospital security review expects
              </h2>
            </motion.div>
  ...
  if (view === "enroll") {
    return (
      <div style={page}>
        ...
        <div style={{ position: "relative", zIndex: 2 }}>
          <Top right={
            <button style={btnGhost} onClick={() => { stopCam(); go("landing"); }}>Cancel</button>
          } />
          <div style={{ maxWidth: 680, margin: "0 auto", padding: "56px 32px 100px" }}>
                ...
                return (
                  <div key={lab} style={{ flex: 1 }}>
                    ...
                    <div style={{
                      ...
                      fontWeight: 700, color: on || done ? T.gold : T.dim, textAlign: "center",
                    }}>
                      {lab}
                    </div>
                  </div>
            ...
            <AnimatePresence mode="wait">
              {step === 0 && (
                <motion.div key="s0" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} style={{ ...glass, padding: 44 }}>
                  ...
                  <label style={{
                    ...
                    <input
                      type="checkbox"
                      checked={consent}
                      onChange={e => { try { sfx.tap(); } catch {} setConsent(e.target.checked); }}
                      style={{ marginTop: 4, accentColor: T.gold, width: 18, height: 18, flexShrink: 0 }}
                    />
                    <span style={{ fontSize: 14, lineHeight: 1.55 }}>
                      I understand and consent to biometric enrolment for hospital system access.
                    </span>
                  </label>
                  <button
                    style={{ ...btnGold, opacity: consent ? 1 : 0.35, pointerEvents: consent ? "auto" : "none" }}
                    onClick={() => { try { sfx.tap(); } catch {} setStep(1); }}
                  >
              ...
              {step === 1 && (
                <motion.div key="s1" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} style={{ ...glass, padding: 44 }}>
              ...
              {step === 2 && (
                <motion.div key="s2" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} style={{ ...glass, padding: 44 }}>
                  ...
                  <div style={{
                    ...
                    />
                    <div style={{
                      position: "absolute", inset: "11% 17%", borderRadius: "50%",
                      border: "1.5px dashed rgba(212,175,55,0.5)", pointerEvents: "none",
                    }} />
                    {camErr && (
                      <div style={{
                  ...
                  <div style={{ textAlign: "center", fontWeight: 600, fontSize: 13, color: faceOn ? T.ok : T.muted, marginBottom: 18 }}>
                    {camErr
                      ? "Camera error — simulated capture on"
                      : !modelsOk
                        ? "Loading face model…"
                        : faceOn
                  ...
                  <div style={{ display: "flex", justifyContent: "center", gap: 14, marginBottom: 26 }}>
                    {[0, 1, 2].map(i => (
                      <div key={i} style={{
                        ...
                        border: `1px solid ${captures[i] ? T.gold : T.line2}`, background: "#060606",
                        flexShrink: 0,
                      }}>
                        {captures[i]
                  ...
                  <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
                    <button
                      ...
                      style={{
                        ...btnGold,
                        opacity: captures.length >= 3 || (!faceOn && !camErr && modelsOk) ? 0.4 : 1,
                        pointerEvents: captures.length >= 3 || (!faceOn && !camErr && modelsOk) ? "none" : "auto",
                      }}
                    >
                    ...
                    {captures.length >= 3 && (
                      <button
                        style={btnTeal}
                        onClick={() => { try { sfx.tap(); } catch {} stopCam(); setStep(3); }}
                      >
                        Continue
                      </button>
                    )}
              ...
              {step === 3 && (
                <motion.div key="s3" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={{ ...glass, padding: 44, textAlign: "center" }}>
                  ...
                  <button style={btnGold} onClick={finishEnrol}>
              ...
              {step === 4 && (
                <motion.div key="s4" initial={{ scale: 0.94, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ ...glass, padding: 56, textAlign: "center" }}>
                  ...
                  <div>
                    <button
                      style={btnTeal}
                      onClick={() => { try { sfx.tap(); } catch {} setView("login"); setPhase("idle"); }}
                    >
  ...
  if (view === "login") {
    ...
    return (
      <div style={page}>
        ...
        <div style={{ position: "relative", zIndex: 2 }}>
          <Top right={
            <button style={btnGhost} onClick={() => { stopCam(); go("landing"); }}>Home</button>
          } />
          <div style={{ maxWidth: 560, margin: "0 auto", padding: "56px 32px 100px" }}>
            <div style={{ ...glass, padding: 44 }}>
              ...
              <div style={{
                ...
                <video
                  ...
                  style={{
                    ...
                    transform: "scaleX(-1)", display: phase === "scanning" ? "block" : "none",
                  }}
                />
                {phase === "scanning" && (
                  <motion.div
                ...
                {phase !== "scanning" && (
                  <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center" }}>
                    {phase === "result" && tier
                      ? <tier.Icon size={68} color={tier.c} />
              ...
              <p style={{ textAlign: "center", fontWeight: 600, fontSize: 13, color: T.muted, marginBottom: 24 }}>
                ...
                {phase === "result" && tier && (
                  <span style={{ color: tier.c, fontSize: 15 }}>{tier.label}</span>
                )}
              </p>
              ...
              {phase === "result" && risk && tier && (
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
                  <div style={{ textAlign: "center", marginBottom: 22 }}>
                    ...
                    <div style={{
                      fontSize: 64, fontWeight: 600, fontFamily: "IBM Plex Mono, monospace",
                      color: tier.c, lineHeight: 1.1,
                    }}>
                      {scoreAnim}
                    </div>
                  </div>
                  ...
                  <div style={{
                    background: "#060606", borderRadius: 16, padding: 18,
                    border: `1px solid ${T.line2}`, marginBottom: 18,
                  }}>
                    {risk.rows.map(b => (
                      <div key={b.l} style={{ marginBottom: 14 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 5 }}>
                          <span style={{ fontWeight: 600 }}>{b.l}</span>
                          <span style={{ fontFamily: "IBM Plex Mono, monospace", color: b.v > 15 ? T.bad : T.muted }}>
                            +{b.v}
                          </span>
                        </div>
                        <div style={{ height: 4, background: "rgba(255,255,255,0.05)", borderRadius: 99, overflow: "hidden" }}>
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${Math.min(100, b.v * 2.5)}%` }}
                            transition={{ duration: 0.8 }}
                  ...
                  {tier.k === "med" && (
                    <div style={{
                      ...
                      <div style={{ fontWeight: 700, color: T.warn, marginBottom: 10, fontSize: 13 }}>
                        STEP-UP VERIFICATION
                      </div>
                      {!otpOn
                        ? <button style={btnGhost} onClick={() => { try { sfx.tap(); } catch {} setOtpOn(true); }}>Send OTP</button>
                        : (
                          <div style={{ display: "flex", gap: 10 }}>
                            <input
                              style={{ ...inp, marginTop: 0, letterSpacing: 8, fontFamily: "IBM Plex Mono, monospace" }}
                              maxLength={6}
                              value={otp}
                  ...
                  {tier.k === "high" && (
                    <div style={{
                      ...
                      <button
                        style={{ ...btnGhost, marginTop: 14, color: T.bad, borderColor: "rgba(248,113,113,0.4)" }}
                        onClick={() => { try { sfx.tap(); } catch {} setToast("Incident filed with Security Admin"); }}
                      >
                  ...
                  <button
                    ...
                    onClick={() => { try { sfx.tap(); } catch {} setPhase("idle"); setRisk(null); }}
                  >
            ...
            {enrolled.length === 0 && (
              <p style={{ marginTop: 12, fontSize: 12, color: T.warn }}>
                No enrolment — scans will score high risk.
              </p>
            )}
          </div>
        </div>
      ...
      <div style={{ ...page, display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 2, display: "flex", flexDirection: "column", height: "100%" }}>
          ...
          <header style={{
            ...
            background: "rgba(3,3,3,0.9)", backdropFilter: "blur(20px)", flexShrink: 0, flexWrap: "wrap", gap: 12,
          }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
              <div style={{
                ...
                letterSpacing: "0.12em", background: "rgba(52,211,153,0.12)", color: T.ok,
          ...
          <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
            ...
            <aside style={{
              ...
              background: T.bg2, padding: "28px 16px", display: "flex",
              ...
              {nav.map(n => (
                <motion.button
                  key={n.id}
                  whileHover={{ x: 4 }}
                  onClick={() => { try { sfx.tap(); } catch {} setDashTab(n.id); setPatient(null); }}
                  style={{
              ...
              <button onClick={() => go("iterations")} style={{ ...btnGhost, width: "100%", justifyContent: "center", marginTop: 8 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 28 }}>
                {[
                  ...
                  <div key={s.l} style={{ ...glass, padding: "22px 24px" }}>
                    <div style={{ fontSize: 10, letterSpacing: "0.18em", color: T.dim, fontWeight: 700, marginBottom: 10 }}>
                      {s.l.toUpperCase()}
                    </div>
                    <div style={{ fontSize: 26, fontWeight: 600, color: s.c || T.text, letterSpacing: "-0.02em" }}>
                      {s.v}
                    </div>
                  </div>
                ))}
                ...
                <>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 28 }}>
                    <div style={{ ...glass, padding: 24, borderColor: "rgba(248,113,113,0.22)" }}>
                      <div style={{ fontSize: 10, color: T.bad, fontWeight: 700, letterSpacing: "0.16em", marginBottom: 12 }}>
                        BEFORE — LEGACY ACCESS
                      </div>
                      ...
                      <div style={{ fontSize: 13, color: T.muted, lineHeight: 1.65 }}>
                        Password reuse, tailgating, no contextual risk, weak attribution on who opened which record.
                      </div>
                    </div>
                    <div style={{ ...glass, padding: 24, borderColor: "rgba(52,211,153,0.28)" }}>
                      <div style={{ fontSize: 10, color: T.ok, fontWeight: 700, letterSpacing: "0.16em", marginBottom: 12 }}>
                        AFTER — THIS PLATFORM
                      </div>
                      <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Biometric + AI risk score</div>
                      <div style={{ fontSize: 13, color: T.muted, lineHeight: 1.65 }}>
                        Per-person liveness, device/location/time signals, step-up MFA, immutable access log.
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: patient ? "1fr 340px" : "1fr", gap: 20 }}>
                    <div style={{ ...glass, overflow: "hidden" }}>
                      <div style={{ padding: "18px 24px", borderBottom: `1px solid ${T.line2}`, fontWeight: 700, fontSize: 14, letterSpacing: "0.04em" }}>
                        Patient Records
                      </div>
                      {loadingTable ? (
                        <div style={{ padding: 28 }}>
                          {[1, 2, 3, 4, 5].map(i => (
                            <div key={i} style={{
                              height: 48, marginBottom: 12, borderRadius: 10,
                              background: "rgba(255,255,255,0.03)", animation: "shimmer 1.2s infinite",
                            }} />
                          ))}
                        </div>
                      ) : (
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                          <thead>
                            <tr style={{ color: T.dim, textAlign: "left" }}>
                              {["Name", "ID", "Ward", "Admitted", "Status"].map(h => (
                                <th key={h} style={{ padding: "14px 20px", fontSize: 10, letterSpacing: "0.14em", fontWeight: 700 }}>
                                  {h}
                                </th>
                              ))}
                            </tr>
                          ...
                          <tbody>
                            {PATIENTS.map(p => (
                              <tr
                                key={p.id}
                                onClick={() => { try { sfx.tap(); } catch {} setPatient(p); }}
                                style={{
                                  borderTop: `1px solid ${T.line2}`, cursor: "pointer",
                                  background: patient?.id === p.id ? T.goldDim : "transparent",
                                }}
                              >
                    ...
                    {patient && (
                      <motion.div
                        ...
                        style={{ ...glass, padding: 26, alignSelf: "start" }}
                      >
                        ...
                        <div style={{ fontSize: 12, color: T.muted, lineHeight: 1.6, padding: 14, background: "#060606", borderRadius: 12 }}>
                          {patient.notes}
                        </div>
                      </motion.div>
                    )}
              ...
              {dashTab === "security" && (
                <div style={{ ...glass, padding: 36 }}>
                  ...
                  <p style={{ color: T.muted, fontSize: 14, marginBottom: 24 }}>
                    Enrolled reference frames (local demo storage only).
                  </p>
                  <div style={{ display: "flex", gap: 14, marginBottom: 28, flexWrap: "wrap" }}>
                    ...
                    {!(session.captures || []).length && (
                      <div style={{ color: T.dim }}>Re-enrol to attach capture thumbnails to this session.</div>
                    )}
                  </div>
                  <button
                    style={btnGold}
                  ...
                  {enrolled.length === 0 ? (
                    ...
                    enrolled.map((u, i) => (
                      <div key={i} style={{ display: "flex", gap: 16, padding: "16px 24px", borderBottom: `1px solid ${T.line2}`, alignItems: "center" }}>
                        <div style={{
                          width: 44, height: 44, borderRadius: "50%", background: T.goldDim,
                          color: T.gold, display: "grid", placeItems: "center", fontWeight: 700, flexShrink: 0,
                        }}>
                        ...
                        <div style={{ fontFamily: "IBM Plex Mono, monospace", color: T.gold, fontSize: 12 }}>
                          {u.staffId}
                        </div>
                      </div>
                    ))
  ...
  if (view === "ethics") {
    return (
      <div style={page}>
        ...
        <div style={{ position: "relative", zIndex: 2 }}>
          <Top right={
            <button style={btnGhost} onClick={() => go(session ? "dashboard" : "landing")}>Back</button>
          } />
          <div style={{ maxWidth: 760, margin: "0 auto", padding: "64px 32px 100px" }}>
            ...
            {[
              {
                ...
                d: "Liveness uses real in-browser face-api.js detection. Full identity matching is simulated with transparent weights so every score remains explainable. Demo frames stay in localStorage only.",
              },
              {
                t: "Data protection principles",
                d: "Mandatory consent before camera. Minimisation and purpose limitation. Production requires DPIA, encryption, retention limits, and erasure under GDPR-style rules and Sri Lanka's PDPA.",
              },
              {
                t: "Risks in healthcare biometrics",
                d: "False rejection can block a clinician in an emergency — OTP step-up and fallback paths are mandatory. Template breach is irreversible. Matching bias (NIST FRVT) needs human review on borderline scores.",
              },
            ].map((s, i) => (
              <motion.div
                key={i}
                ...
                style={{ ...glass, padding: 32, marginBottom: 16 }}
              >
                  ...
                  <tbody>
                    {audit.map(a => (
                      <tr key={a.id} style={{ borderTop: `1px solid ${T.line2}` }}>
                        ...
                        <td style={{ padding: "14px 20px", color: a.tier === "low" ? T.ok : a.tier === "med" ? T.warn : T.bad }}>
                          {a.outcome}
                        </td>
                      </tr>
                    ))}
